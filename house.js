...
// // const notHouseGenYet = ["US","MI","TX","FL","MA","KS","MT","MN","MS"];

// // const primaryHouseWinnersByState = {
// //     // NE: "dan osborn",
// //     // ME: "graham platner",
// //     // OH: "sherrod brown",
// //     // SD: "julian beaudion",
...
// //         const required = primaryHouseWinnersByState[poll.state];
// //         if (required) {
...
// //     );
// //     console.log("Raw rows (HOUSE):", rows);
...
// //     getData(houseLink).then(outcomes => {
...
// //             };
// //             applyColor("house", state, Object.keys(prob).find(key => prob[key]) + winningParty[0]);
...
// //             });
// //             changeDesc("house", state, string);
// //         }
        
// //         const noElec = ["HI","CA", "NV", "UT", "AZ", "WA", "ND", "MO", "WI", "IN", "PA", "NY", "MD", "VT", "CT", "MO"];
// //         noElec.forEach(element => {
// //             applyColor("house", element, "noElec");
// //             changeDesc("house", element, "No election");
// //         });
...
function groupByHousePollId(rows) {
    const polls = Object.create(null);
    ...
    for (const row of rows) {
        ...
        if (!poll) {
            poll = polls[key] = {
                ...
                start_date: row.start_date,
                end_date: row.end_date,
                sample_size: Number(row.sample_size) || null,
                responses: []
            };
        }
...
function applyCandidateThresholdHouse(polls, minPolls = 3) {
    const counts = countCandidatePollsHouse(polls);

    return polls.map(poll => {
        const allowed = counts[poll.district] || Object.create(null);

        const filteredResponses = poll.responses.filter(r => {
            if (EXCLUDE_RE.test(r.candidate)) return false;

            const count = allowed[r.candidate] || 0;
            return count >= minPolls;
        });
...
function countCandidatePollsHouse(polls) {
    const counts = Object.create(null);
    ...
    for (const poll of polls) {
        ...
        for (const r of poll.responses) {
            ...
            const name = r.candidate;
            counts[district][name] ??= new Set();
            counts[district][name].add(poll.poll_id);
        }
    }

    const out = Object.create(null);
    for (const district in counts) {
        out[district] = Object.create(null);
        for (const c in counts[district]) {
            out[district][c] = counts[district][c].size;
        }
    }
...
async function getHouseData(url) {
    ...
    const parsed = Papa.parse(csvText, {
        ...
        complete(results) {
            ...
            const withDistrict = rows.map(r => {
                if (!r.state || !r.seat_number) return null;

                let seat = String(r.seat_number).trim();
                seat = seat.padStart(2, "0");

                return {
                    ...r,
                    district: `${r.state}${seat}`
                };
            }).filter(Boolean);

            console.log("Rows with district:", withDistrict.length);
            const polls = groupByHousePollId(withDistrict);
            console.log("Total polls:", polls.length);
            const thresholded = applyCandidateThresholdHouse(polls, 2);
            console.log("After thresholding:", thresholded);

            const pollsByDistrict = {};

             for (const poll of thresholded) {
                if (!poll.district) continue;

                if (!pollsByDistrict[poll.district]) {
                    pollsByDistrict[poll.district] = [];
                }
                pollsByDistrict[poll.district].push(poll);
            }

            console.log(
                Object.entries(pollsByDistrict)
                    .map(([d, p]) => `${d}: ${p.length}`)
            );
...
getHouseData("https://www.nytimes.com/newsgraphics/polls/house.csv");